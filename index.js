...
app.use(
  cors({
    origin: [
      "http://localhost:5173",
      "https://travel-guru-a638f.web.app",
      "https://travel-guru-a638f.firebaseapp.com",
...
async function run() {
  try {
    ...
    const CollectionOfPackeges = client
      .db("TravelGuru")
      .collection("packegeDB");
    const CollectionOfCustomerBooking = client
      .db("TravelGuru")
      .collection("bookingDB");
    const CollectionOfCustomerReview = client
      .db("TravelGuru")
      .collection("reviewDB");
    const CollectionOfDestination = client
      .db("TravelGuru")
      .collection("destinationDB");
    ...
    const systemInstruction = `You are the official Travel Guru Assistant. Your goal is to provide a seamless travel planning and booking experience. 

Your capabilities include:
1. **Expert Travel Planning:** Suggesting specific travel packages and tours available on Travel Guru.
2. **Platform Guide:** Explaining how to use the site (e.g., how to book tours, make secure payments, and track bookings).
3. **Content Creator:** Writing engaging, high-quality travel blogs and itineraries that inspire users to explore.
4. **Professional Tone:** Maintain a tone that is adventurous, helpful, and trustworthy.

When writing blogs, ensure they include catchy titles, vivid descriptions, and clear calls-to-action (CTAs) to check out Travel Guru's packages.`;
